...
var DEFAULT_CLASSES = {
    ...
    dropdownItem: "control-autocomplete-suggestion",
    dropdownItem2: "control-autocomplete-suggestion",
    selectedDropdownItem: "control-autocomplete-suggestion-selected",
    inputToken: "token-input-input-token",
    focused: "token-input-focused",
    disabled: "token-input-disabled",
    hint: 'control-autocomplete-hint',
    suggestions: 'control-autocomplete-suggestions',
    suggestion: 'control-autocomplete-suggestion',
    thumbnail: 'control-autocomplete-thumbnail',
    name: 'control-autocomplete-name',
    mail: 'control-autocomplete-mail',
    id: 'control-autocomplete-id',
    listMix: 'list-mix',
    listMixItem: 'list-mix-item',
    listMixInner: 'list-mix-inner',
    close: 'control-autocomplete-close',
    closeIcon: 'icon-close-gray_16_18 show',
...
var DEFAULT_SETTINGS = {
    ...
    zindex: 999,
    resultsLimit: null,
    resultsFormatter: function(id, name, mail, thumbnail) {
        var thumbnailTag = '',
            mailTag = ''
        ...
        if (typeof mail !== 'undefined') {
            mailTag = '<span class="' + DEFAULT_CLASSES.mail + '">' + mail + '</span>'
        }

        return ('<li class="' + DEFAULT_CLASSES.suggestion + '">' + thumbnailTag + '<span class="' + DEFAULT_CLASSES.name + '">' + name + '</span>' + mailTag + '<span class="' + DEFAULT_CLASSES.id + '">' + id + '</span></li>')
    },
    tokenFormatter: function(id, name, mail, thumbnail) {
        var thumbnailTag = '',
            mailTag = ''
        ...
        return ('<li><div class="' + DEFAULT_CLASSES.listMixInner + '"><span class="' + DEFAULT_CLASSES.close + '"><span class="' + DEFAULT_CLASSES.closeIcon + '"></span></span>' + thumbnailTag + '<div class="' + DEFAULT_CLASSES.addedInfo + '"><span class="' + DEFAULT_CLASSES.name + '">' + name + '</span>' + mailTag + '<span class="' + DEFAULT_CLASSES.id + '">' + id + '</span></div></div></li>')
    },
...
$.TokenList = function (input, url_or_data, settings) {
    ...
    var listMix = $(document.createElement('ul')).addClass(settings.classes.listMix)
    listMix.insertAfter(token_list)
    ...
    function insert_token(item) {
        ...
        if (!isAlreadyIn) {
            var $this_token = $(settings.tokenFormatter(item.id, item.name, item.mail, item.avatar_32));
            var readonly = item.readonly === true ? true : false;

            if(readonly) $this_token.addClass(settings.classes.tokenReadOnly);

            $this_token.addClass(settings.classes.token).appendTo(listMix);

            // The 'delete token' button
            if(!readonly) {
                $this_token.find('.' + settings.classes.close).on('click', function() {
                    if (!settings.disabled) {
    ...
    function populate_dropdown (query, results) {
        if(results && results.length) {
            ...
            $.each(results, function(index, value) {
                var this_li = settings.resultsFormatter(this.id, this.name, this.mail, this.avatar_32);
            ...
            if(settings.noResultsText) {
                dropdown.html($(document.createElement('span')).addClass(settings.classes.hint).append(settings.noResultsText));
                $(input).trigger(settings.triggerInviteByMail)
                show_dropdown();
            }
        }
    }
